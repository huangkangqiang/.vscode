...
let languageClient;
let previousAutocompleteSetting;
function activate(context) {
    ...
    previousAutocompleteSetting = cppSettings.get("autocomplete");
...
function handleSettingsChange(onInit) {
    let cppSettings = vscode.workspace.getConfiguration("C_Cpp");
    let autocomplete = cppSettings.get("autocomplete");
    if (!onInit && autocomplete == previousAutocompleteSetting)
        return;
    ...
    langs.forEach((lang) => {
        ...
        if (autocomplete == "Default") {
            if (langConfig == undefined) {
                con.update(lang, { "editor.quickSuggestions": false }, true);
            }
            else if (langConfig["editor.quickSuggestions"] == undefined || !onInit) {
                langConfig["editor.quickSuggestions"] = false;
                con.update(lang, langConfig, true);
            }
        }
        else if (!onInit) {
            if (langConfig != undefined && langConfig["editor.quickSuggestions"] == false) {
    ...
    previousAutocompleteSetting = autocomplete;
}
