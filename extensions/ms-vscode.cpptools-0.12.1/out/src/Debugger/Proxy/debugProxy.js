...
function proxy() {
    util.checkLockFile().
        ...
        if (fileExists) {
            switch (os.platform()) {
                case "win32":
                    return startDebugChildProcess(util.getDebugAdaptersPath("bin/OpenDebugAD7.exe"), process.argv.slice(2), util.getDebugAdaptersPath(""));
                case "linux":
                case "darwin":
                    return startDebugChildProcess(util.getDebugAdaptersPath("OpenDebugAD7"), process.argv.slice(2), util.getDebugAdaptersPath(""));
            }
        }
        else {
