...
const ActiveDocumentChange_type = {
    get method() { return "cpptools/activeDocumentChange"; }
};
const TextEditorSelectionChange_type = {
    get method() { return "cpptools/textEditorSelectionChange"; }
};
const FormatCausesNoChanges_type = {
    get method() { return "cpptools/formatCausesNoChanges"; }
};
const ReloadWindow_Type = { get method() { return "cpptools/reloadWindow"; } };
const ReportNavigation_type = {
    get method() { return "cpptools/reportNavigation"; }
};
const RequestNavigationList_type = {
    get method() { return "cpptools/requestNavigationList"; }
};
const GoToDeclaration_type = {
    get method() { return "cpptools/goToDeclaration"; }
};
...
function activate() {
    ...
    let bugUserSettings = new C_Cpp_Feedback.FeedbackState();
    ...
    let clientOptions = {
        ...
        initializationOptions: {
            ...
            clang_format_fallbackStyle: con.get("clang_format_fallbackStyle"),
            ...
            storage_path: util.extensionContext.storagePath,
            tab_size: editor.get("tabSize"),
            intelliSenseEngine: con.get("intelliSenseEngine"),
            intelliSenseEngineFallback: con.get("intelliSenseEngineFallback"),
    ...
    C_Cpp_Feedback.setupFeedbackHandler(languageClient);
    util.extensionContext.subscriptions.push(C_Cpp_ConfigurationProperties.setupConfigurationProperties(languageClient));
    util.extensionContext.subscriptions.push(languageClient.start());
    ...
    if (!util.extensionContext.globalState.get("CPP.quickSuggestionsCleared", false)) {
        util.extensionContext.globalState.update("CPP.quickSuggestionsCleared", true);
    ...
    statusBarNavigator.tooltip = "C/C++ Navigation";
    ...
    registeredCommands.push(vscode.commands.registerCommand('C_Cpp.ShowReleaseNotes', () => {
        util.showReleaseNotes();
    }));
    registeredCommands.push(vscode.commands.registerCommand('C_Cpp.ToggleErrorSquiggles', () => {
        toggleSetting("errorSquiggles", "Enabled", "Disabled");
    }));
    registeredCommands.push(vscode.commands.registerCommand('C_Cpp.ToggleIncludeFallback', () => {
        toggleSetting("intelliSenseEngineFallback", "Enabled", "Disabled");
    }));
    languageClient.onNotification(ReportNavigation_type, (notificationBody) => {
        if (notificationBody.navigation.startsWith("<def")) {
            addFileAssociations(notificationBody.navigation.substr(4));
            return;
        }
...
function toggleSetting(key, value1, value2) {
    let config = vscode.workspace.getConfiguration("C_Cpp");
    let value = config.get(key);
    config.update(key, value == value1 ? value2 : value1, true);
}
...
function getClangFormatFilenamePath() {
    ...
    return path.resolve(util.getExtensionFilePath("LLVM/bin"), clangformatProcessName);
}
